...
const projectTimeline = [
  ...
  },
  {
    ...
    description: copy(
      'University information AI chatbot project covering end-to-end data collection, curation, fine-tuning, and serving based on Penn State domain data.',
      'Penn State 대학 도메인 데이터를 기반으로 데이터 수집·정제·파인튜닝·서빙까지 수행한 대학 정보 AI 챗봇 프로젝트',
    ),
    roles: {
      en: [
        'Designed and implemented a large-scale PSU domain crawling/cleanup pipeline (requests, BeautifulSoup, failure logs, retry handling, blocked URL filtering)',
        'Built body extraction plus automated Q/A generation pipeline (local LLM/Ollama with forced JSON format, parsing recovery, sample backfill logic)',
        'Automated training dataset conversion (text -> json -> jsonl(messages)) and produced SFT-ready datasets via merge scripts',
        'Executed Phi-3 3.8B domain fine-tuning with QLoRA (4-bit NF4, double quantization, bfloat16 compute)',
        'Built completion-only training and resumable checkpoint experiment workflow with TRL SFTTrainer',
      ],
      ko: [
        'PSU 도메인 웹페이지 대규모 크롤링/정제 파이프라인 설계 및 구현 (requests, BeautifulSoup, 실패 로그/재시도/차단 URL 필터링)',
        '본문 추출 + Q/A 자동 생성 파이프라인 구축 (로컬 LLM/Ollama 기반 JSON 강제 포맷, 파싱 복구, 샘플 보충 로직)',
        '학습 데이터셋 변환 자동화 (text → json → jsonl(messages)) 및 병합 스크립트로 SFT 학습셋 생산',
        'Phi-3 3.8B 도메인 파인튜닝 수행 (QLoRA: 4-bit NF4 + double quantization + bfloat16 compute)',
        'TRL SFTTrainer 기반 completion-only 학습 및 checkpoint 재개형 실험 워크플로우 구축',
      ],
    },
    outcomes: {
      en: [
        'Collected and cleaned about 57,000 webpages and auto-generated 200,000 Q/A pairs',
        'Built a curated instruction dataset of about 90,000 high-quality samples through noise filtering',
        'Secured a 4-bit QLoRA pipeline that can train Phi-3 3.8B on a single CUDA GPU',
        'Established a reproducible experiment framework with checkpoint-based resume/compare flow (step 1000/1500, etc.)',
        'Delivered a chatbot capable of university-domain Q&A (major, admissions, academics, campus info)',
      ],
      ko: [
        '약 57,000개 웹페이지 수집·정제 및 200,000개 Q/A 자동 생성',
        '노이즈 제거/큐레이션을 통해 약 90,000개 고품질 instruction 데이터셋 구축',
        '4-bit 양자화 기반 QLoRA 적용으로 단일 CUDA GPU 환경에서 Phi-3 3.8B 실험 가능한 학습 파이프라인 확보',
        'checkpoint(step 1000/1500 등) 기반 재학습·비교가 가능한 재현형 실험 체계 구축',
        '대학 도메인 질의응답(전공/입학/학사/캠퍼스 정보) 가능한 챗봇 형태로 구현',
      ],
    },
    deliverables: [
      {
  ...
  },
  {
    ...
    items: {
      en: [
        ...
        'Regex',
        'Web Crawling',
        'Body extraction and cleanup pipeline',
        'JSON/JSONL format conversion pipeline',
